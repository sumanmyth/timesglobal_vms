...
const TaskManagementAddPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: TaskPayload = {
      ...
      company_location: formData.companyLocation || undefined, // Send undefined if empty, backend might handle null
    ...
    if (!payload.job_date || !payload.job_id || !payload.job_title || !payload.full_name || !payload.company_name || !payload.rack_number || !payload.encoded_by) {
      setError("Please fill in all required fields: Job Date, Job ID, Job Title, Full Name, Company Name, Rack Number, Encoded By.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (err.data && typeof err.data === 'object') {
        const backendErrors = Object.entries(err.data)
          .map(([key, value]) => {
            const fieldName = key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()); // Convert snake_case to Title Case
        ...
        setError(err.message || 'An unexpected error occurred while adding the task.');
      }
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="flex-grow p-4 sm:p-6 bg-slate-700 bg-opacity-60 backdrop-blur-sm rounded-b-lg shadow-inner_lg overflow-y-auto">
        <form onSubmit={handleSubmit} className="space-y-4 max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="jobId" className={labelStyles}>Job ID:<span className="text-red-400">*</span></label>
              <Input type="text" id="jobId" name="jobId" value={formData.jobId} onChange={handleChange} className={inputStyles} placeholder="Enter Job ID" required />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input type="text" id="fullName" name="fullName" value={formData.fullName} onChange={handleChange} className={inputStyles} placeholder="Enter Full Name" required />
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input type="tel" id="contact" name="contact" value={formData.contact} onChange={handleChange} className={inputStyles} placeholder="Enter Contact Number" />
            </div>
          </div>
          ...
          <div>
            ...
            <Input type="text" id="encodedBy" name="encodedBy" value={formData.encodedBy} onChange={handleChange} className={inputStyles} placeholder="Enter Encoder's Name" required />
          </div>
